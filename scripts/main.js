...
function hit(y, x, arr) {
      ...
      if (element.r >= 5) {
        if (temp == x && element.y == y) {
          ...
          if (arr == arrOfEnemyShips)
            table2.rows[event.path[1].rowIndex].cells[
  ...
  if (
    enemonepalub.innerHTML +
      enemtwopalub.innerHTML +
      enemthreepalub.innerHTML +
      enemfourpalub.innerHTML ==
    0
  ) {
...
function computer() {
  ...
  if (
    myonepalub.innerHTML +
      mytwopalub.innerHTML +
      mythreepalub.innerHTML +
...
function closeAround(x, y, r, palub, tablica) {
  if (r >= 5) {
    for (let i = 1; i <= palub; i++, x++) {
      if (y + 1 < 10) {
        tablica.rows[y + 1].cells[
          x
        ].innerHTML = `<div class="hit">&#9773</div>`;
        hits.push([y, x]);
      }
      if (y - 1 >= 0) {
        tablica.rows[y - 1].cells[
...
function manualPlace() {
  rotate.addEventListener("click", () => {
    ...
    arrOfTableShip.forEach((element) => {
      if (element.selected) element.rotate();
  ...
  function shipfortable(palub, y, x, r) {
    ...
    if (
      arrOfTableShip.some((element) => {
        temp = element;
        return element.selected == true && element.y == y && element.x == x;
      })
    ) {
      if (r >= 5) {
        //horizontal
        ...
        for (let i = 0; i < element.palub; i++) {
          ...
          if (element.r >= 5) {
            if (temp == x && element.y == y) {
              ...
              for (let z = element.palub + element.x - 1; z >= element.x; z--) {
